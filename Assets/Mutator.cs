...
public class Mutator {
    ...
    private float Fitness(GeneContainer geneContainer, Dictionary<string, float> envAspects) {
        ...
        foreach (var gene in affectedGenes) {
            if (envAspects.ContainsKey(gene)) {
                ans += geneContainer.map[gene].Difference(envAspects[gene]);
            }
        }
    ...
    private GeneContainer Mutate(GeneContainer geneContainer) {
        ...
        foreach (var gene in affectedGenes) {
            ...
            var v1 = Mathf.Clamp(ans.map[gene].min + Random.Range(-5, 5), 0, byte.MaxValue);
            var v2 = Mathf.Clamp(ans.map[gene].max + Random.Range(-5, 5), 0, byte.MaxValue);
            ans.map[gene].min = Mathf.Min(v1, v2);
            ans.map[gene].max = Mathf.Max(v1, v2);
            //ans.map[gene] = val;
        }
    ...
    private float Combine(float val1, float val2) {
        return val1 - (int)val1 + (int)((int)val1 & mask) +
                ((int)val2 & (int)(byte.MaxValue ^ mask));
    }
    ...
    private (GeneContainer first, GeneContainer second) CrossOver(GeneContainer father, GeneContainer mother) {
        ...
        foreach (var gene in affectedGenes) {
            Gene.Interval fatherI = new Gene.Interval(father.map[gene]);
            Gene.Interval motherI = new Gene.Interval(mother.map[gene]);

            child1.map[gene].min = Combine(fatherI.min, motherI.min);
            child1.map[gene].max = Combine(motherI.max, fatherI.max);

            child2.map[gene].min = Combine(motherI.min, fatherI.min);
            child2.map[gene].max = Combine(fatherI.max, motherI.max);
        }
    ...
    private List<(GeneContainer gene, float f)> RankFitness(List<GeneContainer> geneContainers, EnvironmentController environmentController, float SP) {
        ...
        for (int i = 0; i < ans.Count; ++i) {
            ans[i] = (gene: ans[i].gene, f: ans[i].f / totalF);
        }
        return ans;
    }
    ...
    public GeneContainer Adapt(GeneContainer geneContainer, EnvironmentController environmentController, List<string> affectedGenes) {
        ...
        Assert.IsTrue(population.Contains(geneContainer), "Initial gene got removed");
    ...
    private void PrintGenes(List<GeneContainer> geneContainers, string text) {
        ...
        foreach (var genes in geneContainers) {
            text += "{";
            foreach (KeyValuePair<string, Gene.Interval> entry in genes.map) {
                text += entry.Key.ToString() + "[" + entry.Value.min + ", " + entry.Value.max + "]";
            }
            text += "} ---- ";
        }
